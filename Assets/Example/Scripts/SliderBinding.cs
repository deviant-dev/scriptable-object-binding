...
namespace Bindings {
	public class SliderBinding : MonoBehaviour {
		...
		private void OnVariableChanged(float value) { m_LastValue = m_Control.value = m_Variable.Progress; }

		private void Update() {
			if (!m_LastValue.Approximately(m_Control.value)) { m_LastValue = m_Variable.Progress = m_Control.value; }
		}
