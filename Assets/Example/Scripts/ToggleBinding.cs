...
namespace Bindings {
	public class ToggleBinding : MonoBehaviour {
		...
		private void OnVariableChanged(bool value) { m_LastValue = m_Control.isOn = m_Variable.Value; }

		private void Update() {
			if (!m_LastValue == m_Control.isOn) { m_LastValue = m_Variable.Value = m_Control.isOn; }
		}
