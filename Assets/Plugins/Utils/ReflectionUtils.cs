...
namespace Deviant.Utils {
	public static class ReflectionUtils {
		...
		public static bool HasProperty(object target, string propertyName) { return target.GetType().FindProperty(propertyName) != null; }
		...
		private static PropertyInfo FindProperty(this Type type, string propertyName) {
			...
			PropertyInfo info;
			try { info = s_PropertyCache[propertyPath]; }
			catch {
				info = s_PropertyCache[propertyPath] = type.GetProperty(propertyName, kAllBindings);

				if (info == null) { Debug.LogWarning("Property not found: " + propertyPath); }
			}

			return info;
		}

		public static T GetPropertyValue<T>(object obj, string propertyName) { return (T) obj.GetType().FindProperty(propertyName).GetValue(obj, null); }

		public static void SetPropertyValue(object obj, string propertyName, object value) { obj.GetType().FindProperty(propertyName).SetValue(obj, value, null); }
	}
}